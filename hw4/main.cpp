...
int main(int argc, char** argv) {
    ...
    // double comm1_time = 0, compute_time = 0, comm2_time = 0;
    for (int step = 0; step < nsteps; ++step) {
        // vector<double> runtimes = simulate_one_step(parts, num_parts, size, rank, num_procs);
        simulate_one_step(parts, num_parts, size, rank, num_procs);
        // if (rank == 0) {
        //     comm1_time += runtimes[0];
        //     compute_time += runtimes[1];
        //     comm2_time += runtimes[2];
        // }
    ...
    if (rank == 0) {
        std::cout << "Simulation Time = " << seconds << " seconds for " << num_parts
                  << " particles.\n";
        // std::cout << "Breakdown: " << comm1_time << '\t' << compute_time << '\t' << comm2_time << endl;
    }
    if (fsave) {
        fsave.close();
    }
